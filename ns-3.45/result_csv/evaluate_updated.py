...
# Models
    ...
    if split == "group_distance" and df["distance"].notna().any():
        groups = df["distance"].to_numpy()
        unique_groups = np.unique(groups[~np.isnan(groups)])
        n_group = len(unique_groups)
        if n_group < 2:
            print("WARNING: distance groups < 2. Falling back to random split.")
            split = "random"
        else:
            gkf = GroupKFold(n_splits=n_group)
            splits = list(gkf.split(df, y, groups=groups))
            split_name = f"GroupKFold(leave-one-distance-out, groups={n_group})"
    else:
        split = "random"

    if split == "random":
        splitter = ShuffleSplit(n_splits=random_splits, test_size=test_size, random_state=seed)
        splits = list(splitter.split(df, y))
        train_pct = int(round((1.0 - test_size) * 100))
        test_pct = int(round(test_size * 100))
        split_name = f"ShuffleSplit(random {train_pct}/{test_pct}, n_splits={random_splits})"
    ...
    for fs_name, cols in feature_sets:
        ...
        for model_name, model in models:
            ...
            # (optional) store out-of-fold-like predictions (average over times a sample appears in test)
...
def main():
    ...
    ap.add_argument("--split", default="random", choices=["random", "group_distance"],
                    help="CV split strategy. 'random' = repeated random splits. 'group_distance' = leave-one-distance-out (if distance exists).")
    ap.add_argument("--random_splits", type=int, default=50,
                    help="Number of repeated random splits for --split random (default: 50)")
    ap.add_argument("--test_size", type=float, default=0.2,
                    help="Test ratio for random split. 0.2 means 20% test / 80% train (default: 0.2)")
    args = ap.parse_args()
